        ...
        <div>
            <div className="mb-4">
                ...
                <div className="mt-2 space-x-2">
                    <button
                        className={`border px-2 py-1 ${filter === "ALL" ? "bg-blue-500 text-white" : ""}`}
                        onClick={() => setFilter("ALL")}
                    >
                        전체
                    </button>
                    <button
                        className={`border px-2 py-1 ${filter === "WAITING" ? "bg-blue-500 text-white" : ""}`}
                        onClick={() => setFilter("WAITING")}
                    >
                        답변 대기 중
                    </button>
                    <button
                        className={`border px-2 py-1 ${filter === "COMPLETED" ? "bg-blue-500 text-white" : ""}`}
            ...
            {filteredInquiries.map(inquiry => (
                <li key={inquiry.id} className="mb-4 p-4 border rounded">
                    ...
                    {inquiry.status === "COMPLETED" && (
                        <span onClick={() => setExpandedResponseId(prevId => (prevId === inquiry.id ? null : inquiry.id))}>답변 완료됨</span>
                    )}

                    {/* Display expanded response if applicable */}
                    {expandedResponseId === inquiry.id && (
                        <div>
                            <h2 className="text-xl font-semibold mb-2" onClick={() => setExpandedResponseId(null)}>답변 내용</h2>
