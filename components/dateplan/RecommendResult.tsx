...
const RecommendResult: React.FC<RecommendResultProps> = ({ results }) => {
  if (!results || results.length === 0) {
    return <div className="text-center text-gray-600 my-8">코스 추천 결과가 없습니다</div>;
  }
            ...
            {result.restaurant_prediction && Array.isArray(result.restaurant_prediction) ? (
              result.restaurant_prediction.map((restaurant: any, restaurantIndex: number) => (
                <div key={restaurantIndex} className="mb-4">
                  <h2 className="text-lg font-semibold">{restaurant.사업장명 || "-"}</h2>
                  <p className="text-gray-500">{restaurant.소재지전체주소 || "-"}</p>
                  <p className="text-gray-500">{restaurant.업태구분명 || "-"}</p>
                  <div className="flex">
                  ...
                  {restaurantIndex < result.restaurant_prediction.length - 1 && (
