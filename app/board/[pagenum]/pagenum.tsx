...
function BoardDetail(props: any) {
  ...
  const [commentListShow, setCommentListShow] = useState<boolean>(
    props.commentListShow
  );
  ...
  const fetchData = useCallback(async () => {
    try {
      const response = await SendData(
        "GET",
        `/boards/${props.selectedBoard.bid}/comments`,
        null,
        "fetch comment"
      );
  ...
  return (
    <div>
      ...
      <div className="text-sm text-gray-400 mb-2">
        작성일 {props.formatDate(props.selectedBoard.bcreatedAt)}
      </div>
      <div className="text-sm text-gray-400 mb-2">
        최근 수정 {props.formatDate(props.selectedBoard.bupdatedAt)}
      </div>
      <div className="text-sm text-gray-400 mb-2">
        조회수 {props.selectedBoard.bviews}
      </div>
      <br />
        ...
        <button onClick={() => setCommentListShow(!commentListShow)}>
          <div className="hover:text-pink-400">
            <svg
              ...
              strokeWidth={1.5}
              stroke="currentColor"
              className="w-6 h-6 active:text-pink-400 hover:text-pink-400"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      {commentListShow && (
        <div
          className={`comment-list ${
            commentListShow ? "p-4 rounded-lg shadow-md" : ""
          }`}
        >
                    ...
                    <div className="flex justify-between items-center">
                      <div className="flex">
                        ...
                        <span className="text-sm text-gray-500">
                          {props.formatDate(comment.ccreatedAt)}
                        </span>
                      </div>
                      {session &&
                        session.user &&
                        session?.user.name === comment.nickName && (
      ...
      {!isLoading && (
        <div>
          <div className=" space-y-0">
            {boards.length === 0 && (
              <div className="flex justify-center p-48">게시글이 없습니다.</div>
            )}
            {boards &&
              boards.map((board: Board) => (
                <React.Fragment key={board.bid}>
                  ...
                  >
                    ...
                    <div className="w-4/12 text-center flex items-center justify-center">
                      <span className=" hover:underline">{board.btitle} </span>
                      {board.commentList.length != 0 && (
                        <div className="text-xs text-pink-700 ml-3 hover:none">
