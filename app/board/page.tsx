...
function Logined(props: any): any {
  ...
  useEffect(() => {}, [findingMethod, findStr]);
  ...
  const getPages = useCallback(async () => {
    try {
      const pagecount: number = await SendData(
        "GET",
        `/boards`,
        null,
        "fetch page count"
      ...
      <>
        <div className="items-center max-w-6xl mx-auto flex-col justify-center min-h-screen">
          ...
          <div className="flex justify-items-end py-2">
            <button
              onClick={() => {
                setInputFindingMethod("1");
                handleSearchForm();
              }}
              className={
                showSearchForm ? "search active" : "search object-right"
              }
            >
              <GlassIcon />
            </button>
            ...
            <div
              className={`search-form 
                            ${
                              showSearchForm
                                ? "active flex items-center w-max space-x-3 border rounded border-gray-300 hover:border-gray-400"
                                : ""
                            }`}
            >
              {showSearchForm && (
                <>
                  <div className="w-30">
                    <FindingMethod
                      inputFindingMethod={inputFindingMethod}
                      list={findingMethods}
                  ...
                  <div
                    className="p-1 w-3"
                    onClick={() => {
                      setFindingMethod(inputFindingMethod);
                      setFindStr(inputFindStr);
                    }}
                  >
                    <span className="h-1 w-1 text-gray-300 hover:text-gray-400">
                      <GlassIcon />
                    </span>
                  </div>

                  <div
                    className="pr-8 pl-1 w-3"
          ...
          <div className="flex mt-4 space-x-2 justify-center items-center z-0">
            ...
            <p className="bg-pink-500 text-white p-2 rounded relative inline-flex items-center rounded-md border border-gray-300 px-4 py-2 text-sm font-medium">
              {pages + 1}
            </p>
            {pageCount - 1 > pages && (
              <button
            ...
            {pageCount > pages && !(pageCount - 1 == pages) && (
              <button
                onClick={() => setPages(pageCount - 1)}
                className="relative inline-flex items-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-medium text-gray-700 hover:bg-gray-50"
              >
                {">>"}
              </button>
          ...
          <div
            className="fixed bottom-5 right-5 bg-pink-500 hover:bg-black text-white font-bold py-2 px-4 rounded-full"
