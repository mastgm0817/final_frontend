...
const UserList = () => {
  ...
  const getUsers = async () => {
    try {
      const response = await axios.get("http://180.150.207.73:8080/api/users");
  ...
  const createUser = async () => {
    try {
      await axios.post("http://180.150.207.73:8080/api/users", newUser);
      getUsers();
      setNewUser([]);
    } catch (error) {
      console.log(error);
  ...
  const deleteUser = async (uid) => {
    try {
      await axios.delete(`http://180.150.207.73:8080/api/users/${uid}`);
  ...
  const updateUser = async (uid, updatedUser) => {
    try {
      await axios.put(`http://180.150.207.73:8080/api/users/${uid}`, updatedUser);
      getUsers();
    } catch (error) {
      console.log(error);
    }
